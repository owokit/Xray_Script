...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --enable-swap)
      ENABLE_SWAP="true"; shift 1;;
    --swap-size)
      SWAP_SIZE="$2"; shift 2;;
    --no-swap)
      ENABLE_SWAP="false"; shift 1;;
    --remove-swap)
      REMOVE_SWAP="true"; shift 1;;
    --base-dir)
      BASE_DIR="$2"; shift 2;;
    --tls-cert-mode)
...
detect_swap_total_mib() {
  ...
  if [[ -r /proc/swaps ]]; then
    ...
    swap_kib="$(swapon --show --noheadings 2>/dev/null | awk '{sum += $3} END {print sum + 0}')"
  fi
...
ensure_swap() {
  ...
  local swap_mode="${ENABLE_SWAP:-true}"
...
PROFILE_LIB_URL="https://raw.githubusercontent.com/owokit/Xray_Script/main/xray-profiles-lib.sh?nocache=$(date +%s)"
...
for attempt in 1 2 3; do
  if curl -fsSL -H 'Cache-Control: no-cache' -H 'Pragma: no-cache' "$PROFILE_LIB_URL" -o "$PROFILE_LIB"; then
    download_ok="true"
    break
  fi
...
[Unit]
Description=Xray Server - ${PROFILE_DISPLAY_NAME}
...
log_info "$(t "安装后建议执行以下检查:" "Recommended post-install checks:")"
if command -v systemctl >/dev/null 2>&1; then
  log_info "  systemctl status ${SERVICE_NAME} --no-pager"
fi
if [[ -n "${REALITY_PORT:-}" ]]; then
  log_info "  ss -lntup | grep -E ':${REALITY_PORT}|xray'"
fi
if [[ "$PROFILE" == "reality-kcp" && -n "${VMESS_KCP_PORT:-}" ]]; then
  log_info "  ss -lunp | grep -E ':${VMESS_KCP_PORT}|xray'"
fi
log_info "  free -h"
log_info "  swapon --show"

# Install xray management command
XRAY_MANAGER_URL="https://raw.githubusercontent.com/owokit/Xray_Script/main/linux/xray-manager.sh?nocache=$(date +%s)"
...
if [[ ! -f "$XRAY_CMD_PATH" ]] || [[ "${FORCE_INSTALL_MANAGER:-false}" == "true" ]]; then
  ...
  if curl -fsSL -H 'Cache-Control: no-cache' -H 'Pragma: no-cache' "$XRAY_MANAGER_URL" -o "$XRAY_CMD_PATH" 2>/dev/null; then
